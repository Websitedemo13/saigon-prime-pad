...
import { properties, propertyTypes, districts, priceRanges } from "@/data/properties";
...
export default function AllProjects() {
  ...
  const filtered = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "area-asc": result.sort((a, b) => a.areaNum - b.areaNum); break;
      case "area-desc": result.sort((a, b) => b.areaNum - a.areaNum); break;
    }

    return result;
  }, [search, selectedType, selectedDistrict, selectedPrice, sortBy]);
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-1 bg-muted rounded-lg p-1">
              <button
                ...
                className={`p-2 rounded-md transition-colors ${viewMode === "grid" ? "bg-background shadow-sm text-primary" : "text-muted-foreground"}`}
              >
                <LayoutGrid className="w-4 h-4" />
              </button>
              <button
                onClick={() => setViewMode("list")}
                className={`p-2 rounded-md transition-colors ${viewMode === "list" ? "bg-background shadow-sm text-primary" : "text-muted-foreground"}`}
              >
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((p, i) => (
                <Card
                  key={p.id}
                  className="card-hover border-0 shadow-card overflow-hidden group animate-scale-in"
                ...
                >
                  <Link to={`/du-an/${p.slug}`} className="block">
                    <div className="relative overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3 flex gap-2">
                        <Badge className="bg-gradient-primary text-primary-foreground text-xs shadow-lg">
                          {p.status}
                        </Badge>
                      </div>
                      <div className="absolute top-3 right-3">
                        <Badge variant="secondary" className="bg-white/90 text-foreground font-bold text-xs shadow-lg">
                  ...
                  <CardContent className="p-5">
                    <Badge variant="outline" className="text-primary border-primary text-xs mb-2">
                      {p.type}
                    </Badge>
                    ...
                    <div className="flex items-center text-muted-foreground text-sm mb-3">
                      <MapPin className="w-3.5 h-3.5 mr-1 shrink-0" />{p.location}
                    </div>
                    <div className="flex items-center gap-4 text-sm text-muted-foreground mb-3">
                      {p.areaNum > 0 && <span className="flex items-center gap-1"><Square className="w-3.5 h-3.5 text-primary" />{p.area}</span>}
                    ...
                    <div className="flex items-end justify-between">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{p.pricePerM2}</div>
                      </div>
                      <Button size="sm" className="btn-primary text-xs" asChild>
                        <Link to={`/du-an/${p.slug}`}>Chi tiết</Link>
                      </Button>
                    </div>
                  </CardContent>
                </Card>
                      ...
                      <CardContent className="p-6 flex-1 flex flex-col justify-between">
                        ...
                        <div className="flex items-end justify-between pt-3 border-t border-border">
                          <div className="flex items-center gap-6 text-sm text-muted-foreground">
                            {p.areaNum > 0 && <span className="flex items-center gap-1"><Square className="w-4 h-4 text-primary" />{p.area}</span>}
